...
- **SiteGround GoGeek NO expone el binario `crontab` por SSH** (verificado 2026-04-30, Bloque 02 Paso 7). La gestión de crons es exclusivamente vía Site Tools UI. El `* * * * * php artisan schedule:run` se configurará manualmente desde la UI. Pendiente confirmar que el dropdown UI ofrece granularidad "cada 1 minuto"; si la mínima fuese ≥ 5 min, queue jobs y notificaciones webpush sufrirían latencia adicional (degradación aceptable para mantenimiento mensual, no bloqueante; si la mínima fuese ≥ 15 min sería bloqueante y obligaría a reabrir esta decisión — evaluar Supervisor o cambio de hosting). Documentado también en `docs/security.md §Bloque 02`.
