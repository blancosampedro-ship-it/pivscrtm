...
| 2026-07-19 | **Tablas admin: densidad productive real + fila clicable + kebab. Se elimina la columna Acciones sticky** (revierte el enfoque de PR #67). Padding de celda compactado a 8px (filas 36–41px medidas, antes 56px+), sidebar 220px (antes 320px default Filament, DESIGN §Layout ya lo especificaba), acciones = fila clicable que abre Ver (slide-over) + ActionGroup ⋮ (mismo patrón que PivResource). Notas oculta por defecto (toggleable). | La columna sticky tapaba el badge Estado en reposo y leía como parche pegado a la derecha. Root cause: la tabla desbordaba horizontalmente. Solución elegante = eliminar la causa: con densidad Carbon real y sidebar a spec, la tabla de Averías cabe entera a 1280px (overflow 0 medido) y no hay nada que fijar ni maquillar. Si una tabla ancha desborda, las acciones hacen scroll con ella (como GitHub). Verificado con arnés visual light+dark. |
