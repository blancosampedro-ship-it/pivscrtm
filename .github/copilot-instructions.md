...
- **Filament Infolist con relaciones nullable**: NUNCA usar dot-notation (`TextEntry::make('relation.field')`) cuando `relation` puede ser null en algún registro de prod. Filament 3 falla con "Infolist has no record() or state() set" cuando `badge()` recibe state null vía dot-notation, y `placeholder()` no rescata el rendering. Usar SIEMPRE state callback explícito: `TextEntry::make('field_label')->getStateUsing(fn ($record) => $record->relation?->field)->placeholder('—')`. Para el callback: usar `getStateUsing(closure)` (lazy, recibe `$record`), NO `state(closure)` (este último establece state literal y no resuelve el record). Aplica también al closure de `->infolist(fn (Infolist $infolist) => ...)` en `ViewAction`: el parámetro DEBE llamarse exactamente `$infolist` — Filament resuelve los args de los closures por **nombre**, no por tipo; usar `$i` o cualquier otro nombre rompe la inyección y la infolist se queda sin record. Ver Bloque 08c (caso real: avería sin asignación → infolist crash + cualquier `make('...')` falla porque la infolist nunca recibió record).
...
- **Filament 3 RelationManagers en ViewRecord pages — NO USAR**: combinación rota en Filament 3.2 + Livewire 3 contra modelos legacy con primary key custom (`piv_id`, `averia_id`...). Síntoma: POST `/livewire/update` durante lazy-mount fase 2 (`SupportLazyLoading::__lazyLoad`) crashea con `TypeError: Cannot use "::class" on null` en `HasRecords::76`. Causa raíz: cuando Livewire reconstruye el RM desde el snapshot del placeholder, llama `mount()` sin re-bindear `public Model $ownerRecord`, por lo que `bootedInteractsWithTable` cachea `tableFiltersForm` con un `Table::getModel()` que devuelve null. NO se arregla con `$isLazy = false` (mueve el crash al render inicial, mismo error). NO se arregla quitando `->filters([...])` (HasFilters cachea el form siempre). **Workaround Bloque 08g: reemplazar el RelationManager por una tabla server-rendered en Blade**, inyectada en la página vía override de `protected static string $view`. Ejemplo: [`AveriasRelationManager` eliminado, reemplazado por `resources/views/filament/resources/piv-resource/partials/averias-table.blade.php` + `ViewPiv::$view` custom. Tabla HTML con clases Tailwind (sin componentes Livewire), eager loading manual con `->with([...])`, sin filtros interactivos. Para filtrado avanzado, usar página Filament standalone (no RM). Ver Bloque 08g.
